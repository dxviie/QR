<script lang="ts">
		...
		if (qrCount !== prevQrCount) {
			(async () => {
				try {
					...
					davidSVG = textToPath('David', 0, 0, glyphMap);
	...
	function textToPath(
		...
		x = 0,
		y = 0,
		glyphMap: Map<string, { pathData: string; width: number }>,
		scale = 0.0025
	) {
	...
	{#if imageUrl}
		...
		>
			...
			<g id="cutting-lines">
				...
				<!--   Horizontal Top   -->
				<path
					d={`M ${startX + offsetX - 10},${startY + offsetY} L ${startX + offsetX + 3},${startY + offsetY}`}
					stroke="black"
					fill="none"
				/>
				<path
					d={`M ${startX + offsetX + cardSizeX * cardsX - 3},${startY + offsetY} L ${startX + offsetX + cardSizeX * cardsX + 10},${startY + offsetY}`}
					stroke="black"
					fill="none"
				/>
				<!--   Horizontal Bottom   -->
				<path
					d={`M ${startX + offsetX - 10},${startY + offsetY + cardSizeY * cardsY} L ${startX + offsetX + 3},${startY + offsetY + cardSizeY * cardsY}`}
					stroke="black"
					fill="none"
				/>
				<path
					d={`M ${startX + offsetX + cardSizeX * cardsX - 3},${startY + offsetY + cardSizeY * cardsY} L ${startX + offsetX + cardSizeX * cardsX + 10},${startY + offsetY + cardSizeY * cardsY}`}
					stroke="black"
					fill="none"
				/>
				<!--   Vertical Left   -->
				<path
					d={`M ${startX + offsetX},${startY + offsetY - 10} L ${startX + offsetX},${startY + offsetY - 5}`}
					stroke="black"
					fill="none"
				/>
				<path
					d={`M ${startX + offsetX},${startY + offsetY + cardSizeY * cardsY + 5} L ${startX + offsetX},${startY + offsetY + cardSizeY * cardsY + 10}`}
					stroke="black"
					fill="none"
				/>
				<!--   Vertical Right   -->
				<path
					d={`M ${startX + offsetX + cardSizeX * cardsX},${startY + offsetY - 10} L ${startX + offsetX + cardSizeX * cardsX},${startY + offsetY - 5}`}
					stroke="black"
					fill="none"
				/>
				<path
					d={`M ${startX + offsetX + cardSizeX * cardsX},${startY + offsetY + cardSizeY * cardsY + 5} L ${startX + offsetX + cardSizeX * cardsX},${startY + offsetY + cardSizeY * cardsY + 10}`}
					stroke="black"
					fill="none"
				/>
			</g>
				...
				{#each Array(cardsY) as _, row}
					{#each Array(cardsX) as _, col}
						{#if qrCodes && qrCodes[row * cardsX + col]}
							<g
								transform={orientation === 'landscape'
									? `translate(${startX + offsetX + col * cardSizeX + 5}, ${offsetY + startY + row * cardSizeY + 35})`
									: `translate(${startX + offsetX + col * cardSizeX + 21}, ${offsetY + startY + (row + 1) * cardSizeY - 5})`}
							>
